            ...
            for seq in seqs:
                ...
                for image in images:
                    ...
                    for object_id in range(1,no_objects+1):
                        for df in [1,2]:
                            img_list.append(os.path.join('JPEGImages/',seq.strip(),image))
                            labels.append(os.path.join('Annotations_binary/', seq.strip(), image_id + '_' + str(object_id) + '.png'))
                            deformations.append(os.path.join('Deformations/', seq.strip(), image_id + '_' + str(object_id) + '_d' + str(df) + '.png'))
